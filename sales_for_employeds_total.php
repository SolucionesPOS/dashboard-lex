...
$sales = find_by_sql("SELECT v.fecha, u.name, SUM(i.cantidad) As Cantidad 
FROM partida_temps As i INNER JOIN venta_temps As v On v.id = i.ventas_id 
INNER JOIN users As u on v.users_id = u.id 
INNER JOIN clientes As c On c.id = v.clientes_id 
WHERE v.fecha >= '{$request_date_start}' AND v.fecha <= '{$request_date_end}' GROUP BY v.fecha, u.name ORDER BY v.fecha, u.name asc", true);
