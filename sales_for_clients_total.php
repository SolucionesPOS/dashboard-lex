...
$sales = find_by_sql("SELECT v.fecha, c.cliente, c.nombre, SUM(i.cantidad) As Cantidad FROM ventparts_temp As i INNER JOIN ventas_temp As v On v.id = i.ventas_id  INNER JOIN users As u on v.users_id = u.id INNER JOIN clientes As c On c.cliente = v.cliente WHERE v.sinc = 1 AND v.fecha >= '{$request_date_start}' AND v.fecha <= '{$request_date_end}' GROUP BY  v.fecha, c.cliente, c.nombre
", true);
